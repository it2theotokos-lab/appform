<div class="row g-4 mb-5">
    <h5 class="font-heading mb-3 text-white"><i class="fa-solid fa-list-check me-2"></i> <?= __('Available Forms for Submission') ?></h5>
            ...
            <div class="col-md-4">
                <div class="glass-panel p-4 d-flex flex-column h-100 justify-content-between">
                    <div>
                        ...
                        <p class="text-muted small"><?= htmlspecialchars($form['description'] ?? __('No description')) ?></p>
                    </div>
                    <a href="/forms/<?= htmlspecialchars($form['slug']) ?>" class="btn btn-premium btn-sm w-100 mt-3">
                        <?= __('Fill Form') ?> <i class="fa-solid fa-pen-to-square ms-2"></i>
                    </a>
                </div>
            </div>
        <?php endforeach; ?>
...
<div class="glass-panel p-4">
    <h5 class="font-heading mb-4 text-white"><i class="fa-solid fa-clock-rotate-left me-2"></i> <?= __('Submission History') ?></h5>
                ...
                <?php else: ?>
                    <?php foreach ($mySubmissions as $sub): ?>
                        <tr>
                            ...
                            <td><?= $sub['submitted_at'] ? date('d/m/Y H:i', strtotime($sub['submitted_at'])) : __('Draft') ?></td>
                            <td>
                                <?php
                                ...
                                ?>
                                <span class="badge <?= $badgeClass ?>"><?= htmlspecialchars(__($sub['status'])) ?></span>
                            </td>
                            <td>
