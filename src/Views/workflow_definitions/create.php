<div class="row mb-4 align-items-center">
    <div class="col-md-8">
        <h1 class="h3 m-0 text-soft"><i class="fa-solid fa-diagram-project me-2 text-primary"></i> <?= __('Create Workflow') ?></h1>
        <p class="text-muted m-0"><?= __('Enter basic settings for approval workflow.') ?></p>
    </div>
...
<div class="card p-4" style="max-width: 600px;">
    <form action="/admin/workflows" method="POST">
        ...
        <div class="mb-3">
            <label for="name" class="form-label text-soft"><?= __('Workflow Name') ?></label>
            <input type="text" class="form-control" id="name" name="name" required placeholder="<?= __('e.g. Equipment Damage Report') ?>">
        </div>

        <div class="mb-3">
            <label for="description" class="form-label text-soft"><?= __('Description') ?></label>
            <textarea class="form-control" id="description" name="description" rows="3" placeholder="<?= __('Enter description...') ?>"></textarea>
        </div>

        <div class="mb-3">
            <label for="entity_type" class="form-label text-soft"><?= __('Entity') ?></label>
        ...
        <div class="mb-3" id="doc_template_group">
            <label for="document_template_id" class="form-label text-soft"><?= __('Document Template') ?></label>
        ...
        <div class="mb-3 d-none" id="form_template_group">
            <label for="form_id" class="form-label text-soft"><?= __('Form') ?></label>
        ...
        <button type="submit" class="btn btn-premium w-100 mt-2">
            <i class="fa-solid fa-circle-check me-1"></i> <?= __('Create & Design Steps') ?>
        </button>
    </form>
</div>
