...
<div class="row mb-4 align-items-center document-header-toolbar p-3 rounded border" style="margin-top: -15px; background: var(--color-surface); border-color: var(--color-border) !important;">
    <div class="col-md-6 d-flex align-items-center gap-3">
        <a href="/workflow/tasks" class="btn btn-sm btn-outline-secondary">
            <i class="fa-solid fa-arrow-left me-1"></i> <?= __('Back to Tasks') ?>
        </a>
        <div>
            <h4 class="h5 m-0 text-soft font-heading"><?= \App\Core\View::escape($task['doc_title']) ?></h4>
            <small class="text-muted"><?= \App\Core\View::escape($task['document_number']) ?> | <?= __('Workflow Step') ?>: <?= \App\Core\View::escape($task['step_name']) ?></small>
        </div>
    </div>
    <div class="col-md-6 text-end d-flex align-items-center justify-content-end gap-2">
        ...
        <a href="<?= $pdfUrl ?>" target="_blank" rel="noopener noreferrer" class="btn btn-sm btn-primary">
            <i class="fa-solid fa-file-pdf me-1"></i> <?= __('Document Preview') ?>
        </a>
    </div>
</div>
...
<div class="row">
    ...
    <div class="col-md-8">
        <div class="card p-4 mb-4">
            <h5 class="text-soft mb-3"><i class="fa-solid fa-circle-info text-primary me-2"></i> <?= __('Document Details') ?></h5>
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <span class="text-muted small"><?= __('Document Template') ?>:</span>
                    <div class="text-soft"><?= \App\Core\View::escape($task['template_title']) ?></div>
                </div>
                <div class="col-md-6 mb-3">
                    <span class="text-muted small"><?= __('Document Creator') ?>:</span>
                    <div class="text-soft"><?= \App\Core\View::escape($task['creator_name']) ?></div>
                </div>
                <div class="col-md-6 mb-3">
                    <span class="text-muted small"><?= __('Submission Date') ?>:</span>
            ...
            <h6 class="text-soft mb-2"><i class="fa-solid fa-list-check text-primary me-2"></i> <?= __('Completed Form Values') ?></h6>
        ...
        <!-- History Timeline -->
        <div class="card p-4 mb-4">
            <h5 class="text-soft mb-3"><i class="fa-solid fa-clock-rotate-left text-primary me-2"></i> <?= __('Approval History') ?></h5>
            <div class="table-responsive">
                <table class="table table-hover table-sm mb-0">
                    <thead>
                        <tr>
                            <th><?= __('Workflow Step') ?></th>
                            <th><?= __('Responsible (Manager)') ?></th>
                    ...
                    <tbody>
                        <?php foreach ($history as $h): ?>
                            <tr>
                                ...
                                <td><?= \App\Core\View::escape($h['actor_name'] ?? __('Pending')) ?></td>
                                <td>
                                    ...
                                    <?php elseif ($h['status'] === 'returned'): ?>
                                        <span class="badge bg-warning text-dark"><?= __('RETURNED') ?></span>
    ...
    <div class="col-md-4">
        <div class="card p-4 mb-4">
            <h5 class="text-soft mb-3"><i class="fa-solid fa-signature text-primary me-2"></i> <?= __('Make Decision') ?></h5>
            ...
            <form action="/workflow/tasks/<?= (int)$task['id'] ?>/decide" method="POST" id="decision-form">
                ...
                <?php if ($task['step_type'] === 'signature'): ?>
                    <div class="mb-3">
                        <label class="form-label text-soft small"><?= __('Signature Capture') ?></label>
                        <div class="p-2 rounded text-center border mb-2 position-relative" style="background: var(--color-bg); border-color: var(--color-border) !important;">
                            ...
                            <div class="mt-2">
                                <button type="button" class="btn btn-xs btn-outline-secondary" id="clear-sig-btn"><?= __('Clear') ?></button>
                                <button type="button" class="btn btn-xs btn-outline-info" id="undo-sig-btn"><?= __('Undo') ?></button>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div class="mb-3">
                    <label for="comment" class="form-label text-soft small"><?= __('Comment / Notes') ?></label>
                    <textarea class="form-control" id="comment" name="comment" rows="3" placeholder="<?= __('Enter comments (Required for Rejection/Return)') ?>"></textarea>
                </div>

                <div class="d-grid gap-2">
                    <button type="button" class="btn btn-success" id="btn-approve">
                        <i class="fa-solid fa-circle-check me-1"></i> <?= __('Confirm') ?>
                    </button>
                    
                    <?php if ($task['allow_return']): ?>
                        <button type="button" class="btn btn-warning text-dark" id="btn-return">
                            <i class="fa-solid fa-arrow-rotate-left me-1"></i> <?= __('Return for Corrections') ?>
                        </button>
                    <?php endif; ?>

                    <?php if ($task['allow_reject']): ?>
                        <button type="button" class="btn btn-danger" id="btn-reject">
                            <i class="fa-solid fa-circle-xmark me-1"></i> <?= __('Delete') ?>
                        </button>
                    <?php endif; ?>
                </div>
            </form>
        ...
        <?php if (\App\Core\Auth::role() === 'administrator'): ?>
            ...
            <div id="reassign-modal" class="app-modal d-none" role="dialog" aria-modal="true" style="z-index: 1050; position: fixed; inset: 0; background: rgba(0,0,0,0.7); align-items: center; justify-content: center;">
                <div class="app-modal-dialog border border-warning rounded p-4" style="max-width: 500px; margin: 10% auto; background: var(--color-surface);">
                    <div class="app-modal-header mb-3">
                        <h4 class="text-warning m-0"><i class="fa-solid fa-user-pen me-2"></i> <?= __('Reassign Task') ?></h4>
                    </div>
                    <form action="/workflow/tasks/<?= (int)$task['id'] ?>/reassign" method="POST">
                        <?= \App\Core\Csrf::field() ?>
                        <div class="mb-3">
                            <label for="new_assigned_user_id" class="form-label text-soft small"><?= __('Select New User') ?></label>
                        ...
                        <div class="mb-3">
                            <label for="reassign_reason" class="form-label text-soft small"><?= __('Reassignment Reason') ?></label>
                            <textarea class="form-control form-control-sm" name="reason" rows="3" required placeholder="<?= __('Enter the reason for reassignment...') ?>"></textarea>
                        </div>
                        <div class="d-flex justify-content-end gap-2">
                            <button type="button" class="btn btn-secondary" id="btn-close-reassign"><?= __('Cancel') ?></button>
            ...
            <div id="cancel-wf-modal" class="app-modal d-none" role="dialog" aria-modal="true" style="z-index: 1050; position: fixed; inset: 0; background: rgba(0,0,0,0.7); align-items: center; justify-content: center;">
                <div class="app-modal-dialog border border-danger rounded p-4" style="max-width: 500px; margin: 10% auto; background: var(--color-surface);">
                    <div class="app-modal-header mb-3">
                        <h4 class="text-danger m-0"><i class="fa-solid fa-ban me-2"></i> <?= __('Cancel Workflow') ?></h4>
                    </div>
                    <form action="/workflows/instances/<?= (int)$task['workflow_instance_id'] ?>/cancel" method="POST">
                        <?= \App\Core\Csrf::field() ?>
                        <div class="mb-3">
                            <label for="cancel_reason" class="form-label text-soft small"><?= __('Cancellation Reason (Required)') ?></label>
                            <textarea class="form-control form-control-sm" name="reason" rows="3" required placeholder="<?= __('Enter the reason for cancelling the workflow...') ?>"></textarea>
                        </div>
                        <div class="d-flex justify-content-end gap-2">
                            <button type="button" class="btn btn-secondary" id="btn-close-cancel-wf"><?= __('Close') ?></button>
                            <button type="submit" class="btn btn-danger"><?= __('Cancel Workflow') ?></button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="card p-4">
            <h5 class="text-soft mb-3"><i class="fa-solid fa-comments text-primary me-2"></i> <?= __('Comments & Discussion') ?></h5>
            <div class="comments-list mb-3" style="max-height: 300px; overflow-y: auto;">
                <?php if (empty($comments)): ?>
                    <p class="text-muted small text-center m-0"><?= __('No comments yet.') ?></p>
                <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btn-approve').addEventListener('click', function() {
        if (canvas) {
            ...
            if (isBlank) {
                alert('<?= __('Draw your signature on the surface below using a mouse, touch or stylus.') ?>');
                return;
            }
