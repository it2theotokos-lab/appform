<div class="row">
    ...
    <div class="col-md-4 mb-4">
        <div class="card p-4">
            <h5 class="card-title mb-3" style="color:var(--color-text);"><i class="fa-solid fa-gears me-2 text-primary" aria-hidden="true"></i> Επιλογές & Φίλτρα</h5>
            ...
            <form action="/admin/data-exchange/report" method="POST">
                ...
                <div class="mb-3">
                    <label for="form_id" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Επιλογή Φόρμας</label>
                ...
                <div class="mb-3">
                    <label for="department" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Φίλτρο Τμήματος (LDAP)</label>
                ...
                <div class="mb-3">
                    <label for="date_from" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Από Ημερομηνία</label>
                ...
                <div class="mb-3">
                    <label for="date_to" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Έως Ημερομηνία</label>
                ...
                <div class="d-grid gap-2">
                    <button type="submit" name="preview" class="btn btn-premium">
                        <i class="fa-solid fa-magnifying-glass-chart me-1" aria-hidden="true"></i> Προβολή Στατιστικών
                    </button>
                    <button type="submit" name="download_pdf" class="btn btn-outline-danger">
                        <i class="fa-solid fa-file-pdf me-1" aria-hidden="true"></i> Εξαγωγή Αναφοράς PDF
                    </button>
                </div>
            </form>
        </div>
    ...
    <div class="col-md-8">
        <div class="card p-4">
            <h5 class="card-title mb-4" style="color:var(--color-text);"><i class="fa-solid fa-arrow-right-arrow-left me-2 text-primary" aria-hidden="true"></i> Διαμοιρασμός & Εισαγωγές</h5>
            
            <ul class="nav nav-tabs mb-4" id="dataExchangeTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="export-tab" data-bs-toggle="tab" data-bs-target="#export-pane" type="button" role="tab">Εξαγωγή Δεδομένων</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="import-tab" data-bs-toggle="tab" data-bs-target="#import-pane" type="button" role="tab">Εισαγωγή Δεδομένων</button>
                </li>
            </ul>
            ...
            <div class="tab-content" id="dataExchangeTabsContent">
                ...
                <div class="tab-pane fade show active" id="export-pane" role="tabpanel" aria-labelledby="export-tab">
                    <form action="/admin/data-exchange/export" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="export_entity" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Ενότητα / Πίνακας προς Εξαγωγή</label>
                        ...
                        <div class="mb-3">
                            <label class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Μορφή Αρχείου</label>
                        ...
                        <button type="submit" class="btn btn-premium mt-2">
                            <i class="fa-solid fa-download me-1" aria-hidden="true"></i> Εκτέλεση Εξαγωγής
                        </button>
                    </form>

                    <h6 class="mt-4 mb-3 font-semibold text-soft" style="color:var(--color-text-soft);">Ιστορικό Εξαγωγών</h6>
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                <?php if (empty($exportJobs)): ?>
                                    <tr>
                                        <td colspan="6" class="text-center text-muted py-3" style="color:var(--color-text-muted)!important;">Δεν υπάρχουν καταγεγραμμένες εξαγωγές.</td>
                                    </tr>
                                <?php else: ?>
                ...
                <div class="tab-pane fade" id="import-pane" role="tabpanel" aria-labelledby="import-tab">
                    ...
                    <div class="p-3 mb-4 rounded border" style="background-color:var(--color-bg); border-color:var(--color-border)!important;">
                        <h6 class="font-bold text-soft mb-2" style="color:var(--color-text-soft);"><i class="fa-solid fa-file-arrow-down me-1 text-primary"></i> Λήψη Πρότυπου Αρχείου Εισαγωγής</h6>
                        <p class="text-muted small mb-3" style="color:var(--color-text-muted);">
                            Κατεβάστε το πρότυπο, συμπληρώστε τα δεδομένα χωρίς να αλλάξετε τις επικεφαλίδες και ανεβάστε το ολοκληρωμένο αρχείο.
                        </p>
                        
                        <div class="row align-items-end g-3">
                            <div class="col-md-6">
                                <label for="template_entity" class="form-label font-medium text-soft small" style="color:var(--color-text-soft);">Επιλέξτε Ενότητα</label>
                                <select class="form-select" id="template_entity" style="background-color:var(--color-input-bg); color:var(--color-text); border-color:var(--color-border);">
                                    <option value="">-- Επιλογή Ενότητας --</option>
                                    <?php foreach ($entities as $k => $v): ?>
                                        <?php if ($k !== 'audit_logs'): ?>
                            ...
                            <div class="col-md-6 d-flex gap-2">
                                <button type="button" id="btn-dl-csv" class="btn btn-sm btn-outline-secondary w-100" disabled>
                                    <i class="fa-solid fa-file-csv me-1"></i> Λήψη CSV
                                </button>
                                <button type="button" id="btn-dl-xlsx" class="btn btn-sm btn-outline-primary w-100" disabled>
                                    <i class="fa-solid fa-file-excel me-1"></i> Λήψη Excel
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="/admin/data-exchange/import" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="import_entity" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Προορισμός Εισαγωγής</label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="matching_field" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Πεδίο Ταυτοποίησης (Matching Field)</label>
                                <select class="form-select" id="matching_field" name="matching_field" required style="background-color:var(--color-input-bg); color:var(--color-text); border-color:var(--color-border);">
                                    ...
                                    <option value="username">Username (για χρήστες)</option>
                                    <option value="email">Email (για χρήστες)</option>
                                    <option value="slug">Slug (για φόρμες/ρόλους)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="strategy" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Διαχείριση Διπλότυπων (Duplicate Strategy)</label>
                                <select class="form-select" id="strategy" name="strategy" required style="background-color:var(--color-input-bg); color:var(--color-text); border-color:var(--color-border);">
                                    <option value="skip">Παράκαμψη (Skip)</option>
                                    <option value="update">Ενημέρωση Υπάρχοντος (Update)</option>
                                    <option value="duplicate">Δημιουργία Νέου (Duplicate)</option>
                                    <option value="stop">Διακοπή (Stop)</option>
                                </select>
                            </div>

                            <div class="col-md-6 mb-3">
                                <label for="import_file" class="form-label font-medium text-soft" style="color:var(--color-text-soft);">Επιλογή Αρχείου (CSV, JSON)</label>
                        ...
                        <div class="mb-3 form-check">
                            ...
                            <label class="form-check-label text-soft" for="dry_run" style="color:var(--color-text-soft);">Dry Run (Προσομοίωση χωρίς αλλαγή στη βάση)</label>
                        </div>

                        <button type="submit" class="btn btn-premium">
                            <i class="fa-solid fa-upload me-1" aria-hidden="true"></i> Εκτέλεση Εισαγωγής
                        </button>
                    </form>

                    <h6 class="mt-4 mb-3 font-semibold text-soft" style="color:var(--color-text-soft);">Ιστορικό Εισαγωγών</h6>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                <?php if (empty($importJobs)): ?>
                                    <tr>
                                        <td colspan="8" class="text-center text-muted py-3" style="color:var(--color-text-muted)!important;">Δεν υπάρχουν καταγεγραμμένες εισαγωγές.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($importJobs as $job): ?>
                                        <tr>
                                            ...
                                            <td style="color:var(--color-text);"><?= $job['is_dry_run'] ? 'Ναι' : 'Όχι' ?></td>
                                            <td style="color:var(--color-text);"><?= date('d/m/Y H:i', strtotime($job['created_at'])) ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
