    ...
    public function handle(array $args = []) {
        ...
        if (preg_match('#^/forms/(?P<slug>[a-zA-Z0-9_\-]+)(/(draft|submit))?$#', $path, $matches)) {
            ...
            if ($form && !empty($form['is_public'])) {
                return; // Bypass permission check for public form endpoints
            }
        }
        ...
        if (preg_match('#^/f/(?P<token>[a-f0-9]+)$#', $path, $matches)) {
            ...
            $form = $stmt->fetch(\PDO::FETCH_ASSOC);
            if ($form && !empty($form['is_public'])) {
                return; // Bypass permission check for public token route
            }
        }

        if ($path === '/f/submitted/success') {
            return; // Bypass permission check for public success landing page
        }
